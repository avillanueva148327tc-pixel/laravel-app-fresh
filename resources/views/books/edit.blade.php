<x-layout title="Edit Book: {{ $book->title }}">
    <div class="max-w-4xl mx-auto">
        <div class="mb-8 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-white tracking-tight">Edit Book Details</h1>
                <p class="text-blue-200/60 mt-1">Update information for: <span class="text-white">{{ $book->title }}</span></p>
            </div>
        </div>
            ...
            <div class="bg-white/5 p-8 rounded-2xl glass border border-white/10 space-y-8">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" id="description" rows="4" required
                        class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition">{{ old('description', $book->description) }}</textarea>
                </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="genre" id="genre" required
                            ...
                            @foreach(['Cybersec', 'Programming', 'IT Management', 'IT Architecture', 'DevOps', 'SRE', 'Cloud', 'Fiction', 'Non-Fiction', 'Sci-Fi', 'Fantasy', 'Mystery', 'Biography', 'History', 'Self-Help', 'Thriller', 'Academic'] as $genre)
                                <option value="{{ $genre }}" {{ old('genre', $book->genre) == $genre ? 'selected' : '' }} class="bg-slate-900">{{ $genre }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="space-y-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                    <div class="space-y-2">
                        <label for="cover_image" class="text-sm font-semibold text-white/60 tracking-wider uppercase">Update Cover Image</label>
                        <div class="mt-1 flex items-center gap-4">
                            @if($book->cover_image)
                                <img src="{{ asset('storage/' . $book->cover_image) }}" class="w-12 h-16 rounded-lg object-cover border border-white/10">
                            @endif
                    ...
                    <div class="flex items-center gap-3 bg-white/5 p-4 rounded-xl border border-white/10">
                        <input type="checkbox" name="is_available" id="is_available" value="1" {{ old('is_available', $book->is_available) ? 'checked' : '' }}
                ...
                <div class="pt-6 border-t border-white/10 flex gap-4">
                    <button type="submit" class="px-10 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl transition duration-200 shadow-xl shadow-indigo-500/20">
                        Update Information
                    </button>
                    <a href="{{ route('books.index') }}" class="px-10 py-4 bg-white/5 hover:bg-white/10 text-white font-bold rounded-xl transition border border-white/10">
                        Cancel
                    </a>
                </div>
            </div>
